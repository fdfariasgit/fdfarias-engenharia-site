  ...
  return (
    <header 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isScrolled ? 'bg-[#030712]/90 backdrop-blur-md border-b border-slate-900 py-4 shadow-lg' : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          <div className="hidden sm:flex flex-col ml-2 border-l border-slate-800 pl-3">
        ...
        <div className="hidden md:flex items-center gap-6">
          <a 
            href="https://wa.me/556999650890?text=Olá!%20Vim%20pelo%20site%20e%20gostaria%20de%20falar%20com%20um%20especialista."
            target="_blank"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.nav 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-full left-0 w-full bg-[#050b14] border-b border-slate-900 shadow-xl py-6 px-6 flex flex-col gap-6 md:hidden z-50 animate-in fade-in"
          >
            {navLinks.map((link) => (
              <Link 
                key={link.name}
                ...
                className="text-sm font-bold tracking-widest text-slate-200 hover:text-white uppercase transition-colors"
              >
                {link.name}
              </Link>
            ))}
