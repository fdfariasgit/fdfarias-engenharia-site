...
export function ServicesSection() {
  return (
    <section className="py-12 lg:py-24 bg-transparent relative transition-colors border-t border-slate-900" id="servicos">
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 relative z-10">

        {/* Header da Seção */}
        <div className="text-center max-w-3xl mx-auto mb-8 lg:mb-20">
          <span className="block text-xs font-bold tracking-[0.2em] text-slate-400 uppercase mb-4">
            Expertise Técnica
          </span>
          <motion.h2
            ...
            viewport={{ once: true }}
            className="text-3xl md:text-5xl font-extrabold text-white mb-6 tracking-tight leading-tight transition-colors"
          >
            Soluções de <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-sky-400">Alta Precisão.</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0 }}
            ...
            className="text-base md:text-lg text-slate-400 font-light max-w-xl mx-auto leading-relaxed transition-colors"
          >
        ...
        >
          {servicesData.map((service) => (
            <motion.div
              ...
              className="w-[85vw] sm:w-[400px] md:w-auto shrink-0 snap-center bg-[#0b1329]/30 p-8 md:p-10 border border-slate-900 hover:border-blue-500/30 transition-all duration-500 group relative overflow-hidden rounded-xl"
            >
              ...
              <div className="flex flex-col sm:flex-row items-start gap-6 md:gap-8">
                <div className="p-4 bg-slate-900/60 text-slate-400 group-hover:bg-blue-600 group-hover:text-white border border-slate-900 group-hover:border-blue-500/30 transition-colors duration-500 rounded-lg">
                  <service.icon size={32} strokeWidth={1} />
                </div>

                <div className="flex-1">
                  <h3 className="text-xl font-bold text-white mb-4 tracking-tight group-hover:text-blue-400 transition-colors">{service.title}</h3>
                  <p className="text-slate-400 text-sm mb-8 leading-relaxed font-light transition-colors">
                    {service.description}
                  </p>

                  <ul className="space-y-3">
                    {service.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start md:items-center text-xs text-slate-300 font-medium tracking-wide transition-colors">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-12 lg:mt-20 bg-gradient-to-r from-slate-950 to-[#0b1329]/40 border border-slate-900 hover:border-blue-500/20 p-6 md:p-14 flex flex-col lg:flex-row items-center justify-between relative overflow-hidden rounded-2xl transition-colors shadow-sm"
        >
          <div className="relative z-10 text-center lg:text-left mb-8 lg:mb-0 lg:max-w-xl">
            <h4 className="text-2xl md:text-3xl font-extrabold text-white mb-3 tracking-tight">Equipamento parado?</h4>
            <p className="text-slate-300 font-medium">Atendimento emergencial prioritário para clínicas da região.</p>
          </div>
          <a 
